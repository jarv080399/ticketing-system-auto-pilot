...
- [x] Create `system_settings` table (`key` (string unique), `value` (text), `group` (string, e.g., "general", "branding", "email", "integrations"), `type` (string, boolean, integer, json), `updated_by`, `updated_at`).
- [x] Create `business_hours` table (`day_of_week` (0-6), `is_working_day` (boolean), `start_time` (TIME), `end_time` (TIME)).
- [x] Create `holidays` table (`name`, `date` (DATE), `is_recurring` (boolean)).
- [x] Create `custom_fields` table (`entity_type` (ticket, asset, user), `field_name`, `field_label`, `field_type` (text, number, select, date, checkbox), `options` (JSON nullable for select), `is_required` (boolean), `sort_order`).
- [x] Create `custom_field_values` table (`custom_field_id`, `entity_type`, `entity_id`, `value` (text)).
- [x] Implement models: `SystemSetting`, `BusinessHour`, `Holiday`, `CustomField`, `CustomFieldValue`.
- [x] Seed default system settings:
...
- [x] Seed default business hours (Mon-Fri 9am-5pm).

## Step 2: Backend API (Full-Stack Engineer)

- [x] Expose `GET /api/v1/admin/settings` (grouped by `group`).
- [x] Expose `PATCH /api/v1/admin/settings` (batch-update multiple settings, Admin only, logs changes to `audit_logs`).
- [x] Expose CRUD for Business Hours: `GET, PUT /api/v1/admin/business-hours`.
- [x] Expose CRUD for Holidays: `GET, POST, PUT, DELETE /api/v1/admin/holidays`.
- [x] Expose CRUD for Custom Fields: `GET, POST, PUT, DELETE /api/v1/admin/custom-fields`.
- [x] Create a `SettingsService` cached singleton — reads settings from DB on boot, caches in Redis/file. Invalidate on update.
- [x] Expose `GET /api/v1/admin/system-health`:
    - Database connectivity check.
    - Redis connectivity check.
    - Queue worker status.
    - Disk usage.
    - Last migration status.
- [x] Expose `GET /api/v1/admin/activity-log` (paginated audit log viewer with filters: user, action, entity type, date range).

## Step 3: Frontend Admin Panel (Vue 3)

- [x] Build `AdminLayout.vue` with sidebar navigation:
