<template>
    <div class="space-y-8 pb-10">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-black text-text-main tracking-tight">Custom Fields</h1>
        ...
        <div v-else class="w-full space-y-10">
            <div v-for="entity in ['ticket', 'asset', 'user']" :key="entity" class="bg-surface rounded-2xl border border-glass-border shadow-xl overflow-hidden relative">
                ...
                <div class="px-8 py-5 bg-surface-light border-b border-glass-border flex items-center justify-between">
                    <div>
                        <h2 class="text-base font-black uppercase tracking-widest text-text-main">{{ entity }} Metadata Schemas</h2>
                    ...
                    <div class="hidden justify-center sm:flex w-10 h-10 bg-surface rounded-xl border border-glass-border items-center text-primary shadow-sm hover:scale-110 transition-transform cursor-help relative group/tooltip">
                        ...
                        <div class="absolute bottom-full right-0 mb-3 hidden group-hover/tooltip:block w-72 p-3 bg-gray-800 text-xs text-blue-100 rounded-xl border border-blue-500/30 shadow-2xl z-20 normal-case leading-relaxed font-normal">
                            <div class="font-black text-text-main mb-1">Entity Guide:</div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
                        ...
                        >
                            <div class="flex items-center gap-5">
                                ...
                                <div>
                                    <h3 class="font-black text-text-main text-base">{{ field.field_label }}</h3>
                            ...
                            <div class="flex items-center gap-2 opacity-0 group-hover/card:opacity-100 transition-opacity">
                                <button @click="openModal(field)" class="p-2.5 bg-background shadow-inner border border-glass-border rounded-lg text-text-dim hover:text-text-main hover:bg-surface-light transition-all">
        ...
        <div v-if="showModal" class="fixed inset-0 bg-background/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
            <div class="bg-surface border border-glass-border rounded-2xl w-full max-w-2xl p-8 shadow-[0_0_50px_rgba(0,0,0,0.5)] transform scale-100 transition-all max-h-[90vh] overflow-y-auto">
                <div class="flex items-center gap-3 mb-8 pb-4 border-b border-glass-border">
                    ...
                    <div>
                        <h2 class="text-xl font-black text-text-main">{{ editingId ? 'Edit Mapping Schema' : 'Draft New Schema Key' }}</h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                    <div class="col-span-full">
                        ...
                        <select v-model="form.entity_type" :disabled="editingId" class="w-full bg-background border border-glass-border rounded-xl px-4 py-3 text-sm text-text-main focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner disabled:opacity-50">
                    ...
                    <div class="col-span-full md:col-span-1">
                        ...
                        <input v-model="form.field_label" type="text" placeholder="e.g. Department Name" class="w-full bg-background border border-glass-border rounded-xl px-4 py-3 text-sm text-text-main focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner" />
                    ...
                    <div class="col-span-full md:col-span-1">
                        ...
                        <select v-model="form.field_type" class="w-full bg-background border border-glass-border rounded-xl px-4 py-3 text-sm text-text-main focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner">
                    ...
                    <div class="col-span-full md:col-span-1">
                        ...
                        <input v-model="form.sort_order" type="number" class="w-full bg-background border border-glass-border rounded-xl px-4 py-3 text-sm text-text-main focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner" />
                    </div>
                    ...
                    <div v-if="form.field_type === 'select'" class="col-span-full bg-background/50 border border-glass-border p-4 rounded-xl border-dashed">
                        ...
                        <input 
                            ...
                            class="w-full bg-background border border-glass-border rounded-xl px-4 py-3 text-sm text-text-main focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all shadow-inner" 
                        />
                    ...
                    <div class="col-span-full flex items-center justify-between gap-4 bg-background px-5 py-4 rounded-xl border border-glass-border shadow-inner mt-2">
                        <div>
                            <span class="text-sm font-black text-text-main tracking-wide">Mandatory Field Enforcement</span>
                ...
                <div class="flex justify-end gap-3 mt-10 pt-6 border-t border-glass-border">
                    <button @click="showModal = false" class="px-5 py-2.5 text-sm font-bold text-text-dim hover:text-text-main hover:bg-surface-light disabled:opacity-50 transition-colors rounded-xl">Cancel</button>
